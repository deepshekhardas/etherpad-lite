  ...
  setupGlobalExceptionHandler = () => {
    if (this.globalExceptionHandler == null) {
      this.globalExceptionHandler = (e: any) => {
        ...
        // These are unrelated to the pad and should not show error modals.
        const isBrowserExtensionSource = /^(chrome-extension|moz-extension|safari-extension|safari-web-extension):\/\//i.test(url || '');
        if (isBrowserExtensionSource) {
          console.debug('Ignoring error from browser extension:', url);
          return;
        }
